...
# The buggy impl is SUPPOSED to fail. Tolerate its non-zero exit so it doesn't
# pollute the script's final status — but assert it really did fail (a buggy
# impl that "passes" means the test is broken, which is itself a regression).
if node test.mjs ./src/parse-price.mjs; then
  say "${R}REGRESSION: buggy impl unexpectedly PASSED — the test is no longer catching it.${X}"
  exit 1
fi
pause 2
say ""
...
node test.mjs ./src/parse-price.mjs || true; pause 2
say "${C}agent:${X} ❌ caught before claiming done. Stripping the comma, retrying…"; pause 2
# The fixed impl MUST pass. A failure here is a real regression — surface it
# with a non-zero exit so CI (and a human running this) can't miss it.
if ! node test.mjs ./src/parse-price.fixed.mjs; then
  say "${R}REGRESSION: fixed impl FAILED — this should always pass.${X}"
  exit 1
fi
pause 2
say ""
